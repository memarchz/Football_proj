...
# =========================================================
print("🚀 เริ่มระบบ V5.0 SNIPER: Strict + Momentum Logic...")

urls = [
    "https://www.football-data.co.uk/mmz4281/2526/F1.csv", # Future/Current
...
# =========================================================
# 2. Feature Engineering (เพิ่ม Overall Form)
# =========================================================
print("⏳ กำลังคำนวณฟีเจอร์ใหม่: Market + Strict + Overall Form...")
...
def get_sniper_stats(row, full_data):
    ...
    # ถ้าข้อมูลน้อยไป ให้ return 0 ให้ครบ 10 features ใหม่
    ...
    # 5. 🔥 NEW: Overall Form (ฟอร์มรวม 5 นัดล่าสุด จับ Momentum)
    # ---------------------------------------------------------
    last5_h = past[(past['HomeTeam'] == row['HomeTeam']) | (past['AwayTeam'] == row['HomeTeam'])].tail(5)
    h_overall = 0
    for _, m in last5_h.iterrows():
        if m['HomeTeam'] == row['HomeTeam']: 
            h_overall += 3 if m['FTR']=='H' else (1 if m['FTR']=='D' else 0)
        else: 
            h_overall += 3 if m['FTR']=='A' else (1 if m['FTR']=='D' else 0)

    last5_a = past[(past['HomeTeam'] == row['AwayTeam']) | (past['AwayTeam'] == row['AwayTeam'])].tail(5)
    a_overall = 0
    for _, m in last5_a.iterrows():
        if m['HomeTeam'] == row['AwayTeam']: 
            a_overall += 3 if m['FTR']=='H' else (1 if m['FTR']=='D' else 0)
        else: 
            a_overall += 3 if m['FTR']=='A' else (1 if m['FTR']=='D' else 0)
    # ---------------------------------------------------------
...
# Split Time-Series (85% Train / 15% Test)
...
print(f"\n{'='*80}")
print(f"🎯 V5.0 PERFORMANCE REPORT (Win vs Double Chance)")
print(f"{'='*80}")
...
print(f"{'='*85}")
print(f"{'Range':<12} | {'Matches':<8} | {'Win Correct':<12} | {'Real Win %':<12} | {'Real DC %':<15}")
print("-" * 85)

# กำหนดช่วงคะแนน (Bins) แบบละเอียด 5%
bins = [
    (0.40, 0.45), (0.45, 0.50), 
    (0.50, 0.55), (0.55, 0.60),
    (0.60, 0.65), (0.65, 0.70), 
    (0.70, 0.75), (0.75, 0.80), 
    (0.80, 1.01)  # ช่วงเทพ 80+
]
...
for low, high in bins:
    ...
    for i, p in enumerate(probs):
        ...
        # เช็คว่าคะแนนอยู่ในช่วงนี้หรือไม่
        if low <= conf < high:
            total += 1
    ...
    if total > 0:
        ...
        # ให้เกรดความน่าเชื่อถือ (ปรับเกณฑ์ให้เข้ากับ Range ย่อย)
        grade = ""
        ...
        print(f"{low*100:.0f}-{high*100:.0f}%      | {total:<8} | {correct:<12} | {win_acc:.1f}%      | {dc_acc:.1f}% {grade}")
    else:
        print(f"{low*100:.0f}-{high*100:.0f}%      | {0:<8} | {0:<12} | {'N/A':<12} | {'N/A':<15}")

print(f"{'='*85}")

joblib.dump(eclf, 'model_v5_france.pkl')
print(f"\n💾 บันทึกโมเดล V5.0 เรียบร้อย!")